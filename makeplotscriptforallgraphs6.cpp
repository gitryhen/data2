...
version 6.0 distinquish between p95_ and normal data files.
p95_ files are created by baseline program.
...
#define MAIN 1
#define csvluttable "sensloca.csv"
...
void makeplotscriptwithbaseline(std::string s, const std::string & outputfilename,
				const std::string & datafilename, const int colnumber,
				const int datcolnumber, const std::string baselinenumber,
				const std::string & title);
std::string findMyTitle(std::string filename, std::string csvLUT);
  ...
  if (argc == 1)
    {
      ...
      std::cout << "uses sensloca.csv for cross reference of location.\n";
      std::cout << "cross reference only works on p95_*.csv files\n";
      std::cout << "Usage:\n";
	...
	*/
	...
	std::getline(file, line2); // read the second line to determine if column is empty
	file.close();
	std::vector<std::string> line2split;
	std::vector<std::string> resvec;
	line2split = split(line2, ','); // split second line
	...
	*/
	...
	for (auto el : resvec){
	  if (el.substr(0,3) != "p95") {
	    makeplotscript(el,filename, datafilename, findcolnumber(el, mylut), findMyTitle(filename, csvluttable));}
	  else {
	    ...
	    makeplotscriptwithbaseline(el,filename, datafilename, findcolnumber(el, mylut), findcolnumber(dat, mylut),
				       line2split[findcolnumber(el,mylut)-1], findMyTitle(filename, csvluttable));
	  }
	}
...
void boilerplate(){
 ...
 void makeplotscript(std::string s, const std::string & outputfilename,
		     const std::string & datafilename, const int colnumber,
		     const std::string & title){
  std::cout
...
std::string findMyTitle(std::string filename, std::string csvLUT){
  std::ifstream file( csvLUT );
  std::string singleline {};
  std::vector<std::vector<std::string>> lut;
  std::string myResult;
  std::string threeLetterCode;

  threeLetterCode = filename.substr(0, 4);
  if (threeLetterCode == "p95_")
    {
      threeLetterCode = filename.substr(4,3);
    }
  else
    {
      threeLetterCode = filename.substr(0,3);
    }
  if (!file.is_open())
    {
      /* if there is no lut found */
      myResult = threeLetterCode;
    }
    else{
      /* load lut in vector */
      while(std::getline(file, singleline))
	{
	  lut.emplace_back(split(singleline, ','));
	}
    }
  /* search lut for mating location */
  for (auto & l : lut)
    {
      if (l[0] == threeLetterCode)
	{
	  myResult = l[1];
	  break;
	}
